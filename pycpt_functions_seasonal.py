...
class PyCPT_Args():
	...
	def setup_tgts(self, season, monf=False):
	...
	def __init__(self, cptdir, models, met, obs, station, MOS, xmodes_min, xmodes_max, ymodes_min, ymodes_max, ccamodes_min, ccamodes_max, nmodes, PREDICTAND, PREDICTOR,  tini, tend, fyr, force_download, nla1, sla1, wlo1, elo1, nla2, sla2, wlo2, elo2, shp_file, local_data_dir, season,  use_default='True', localobs="None", lonkey="None", latkey="None", timekey="None", datakey="None", showPlot=True):
		#These are the variables set by the user
		self.local_data_dir = local_data_dir
		self.showPlot = showPlot
		self.models = models
		self.shp_file = shp_file
		self.use_default = use_default #not using anymore
		self.met = met
		self.obs = obs
		self.cptdir = cptdir
		...
		self.force_obs_download = force_download
		self.nla1 = nla1
		self.sla1 = sla1
		self.wlo1 = wlo1
		self.elo1 = elo1
		...
		self.ntrain = 0
		self.fprefix=''
		self.tgts, self.mons, self.monf, self.tgti, self.tgtf = self.setup_tgts(season)
	...
	def getData(self,  tar_ndx, model_ndx, datatype):
		...
		good = False
		if datatype == 'Obs' and self.local_data_dir != "None":
			if not self.force_obs_download:
				if self.local_data_dir != "None":
					try:
						ff=open(self.local_data_dir + "obs_"+self.PREDICTAND+"_"+self.tgts[tar_ndx]+".tsv", 'r')
						s = ff.readline()
				...
				if not good:
					try:
						ff=open('./input/' + "obs_"+self.PREDICTAND+"_"+self.tgts[tar_ndx]+".tsv", 'r')
						s = ff.readline()
					except OSError as err:
						print("\033[1mWarning:\033[0;0m {0}".format(err))
						print("Obs precip file doesn't exist --\033[1mSOLVING: downloading file\033[0;0m")
						self.force_obs_download = True

		if datatype != 'Obs' or self.local_data_dir == "None" or good == False:
			...
			if self.force_download:
				if datatype == 'Obs' and self.force_obs_download:
				...
				elif datatype == 'Obs' and self.force_obs_download:
					if self.fprefix == 'RFREQ':
						fpre = 'RFREQ'
					else:
						fpre = 'PRCP'
	...
	def CPTscript(self, tar_ndx, model_ndx=-1):
		...
		# Opens Y input file
		f.write("2\n")
		if self.local_data_dir == "None" or not os.path.isfile(self.local_data_dir + 'obs_'+self.PREDICTAND+'_'+self.tgts[tar_ndx]+'.tsv'):
			print('Using IRI Obs data')
			file='./input/obs_'+self.PREDICTAND+'_'+self.tgts[tar_ndx]+'.tsv\n'
		else:
			print('Using your Data')
			file = self.local_data_dir + 'obs_'+self.PREDICTAND+'_'+self.tgts[tar_ndx]+'.tsv\n'
			print(file)
		f.write(file)
	...
	def pltdomain(self):
		...
		for i in range(2):
			...
			if self.shp_file == "None" or good == False and self.shp_file != "None":
